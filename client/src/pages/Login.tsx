...
export default function Login() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setPhase("sent");
      // Constant-shape response — backend returns ok:true even for non-admin
      // emails (no enumeration). The user only learns whether their email
      // is admin by clicking the link or not getting one.
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      ...
      >
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-xl font-semibold text-foreground">Anchor admin</h1>
          <p className="mt-1 text-xs text-muted-foreground">
            Operator cockpit. Email link only — admins are pre-provisioned.
          </p>
        </div>
        ...
        {phase === "sent" ? (
          <div className="glass rounded-xl p-6 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              If <span className="text-foreground">{email}</span> is on the admin list, a sign-in link is on its way. Valid for 15 minutes.
            </p>
            {message && (
